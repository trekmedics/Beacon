...
Rails.logger.tagged("#{File.basename(__FILE__)}:#{__LINE__}") { Rails.logger.info "********** Test run (#{ARGV[1]}) *********" }
...
# Create incident
  ...
  help_message = '2_incidents_1_fr'
  location = '1st of 2, 1 FR'
  ...
  incident0 = Incident.all.last
  incident0.subcategory_id = 28
  rp0 = ReportingParty.all.last
...
#   incident.save
  ...
  IncomingMessageService.process_message(frs[0].phone_number, incident0.id.to_s + '.5')
...
#   IncomingMessageService.process_message(frs[5].phone_number, incident.id.to_s + '.15')
  ...
  IncidentManager.instance.request_for_assistance_window_expired(incident0.id)


  # Start second incident
# Create incident
  rp_phone_number = '+18005559001'
  help_message = '2_incidents_1_fr, no fr'
  IncomingMessageService.process_message(rp_phone_number, help_message)
  incident1 = Incident.all.last
  rp1 = ReportingParty.all.last


  puts "\tPart 0: incident0.id: #{incident0.id}"
  puts "\tPart 0: incident1.id: #{incident1.id}"
...
# Responds to needs addl help request
  IncomingMessageService.process_message(frs[2].phone_number, incident0.id.to_s + '.5')
...
# Expire Assistance Request window manually
  IncidentManager.instance.request_for_assistance_window_expired(incident0.id)
  IncidentManager.instance.request_for_assistance_window_expired(incident1.id)

  puts "\tPart 1: incident0.id: #{incident0.id}"
  puts "\tPart 1: incident1.id: #{incident1.id}"
...
incident0 = Incident.find(incident0.id)
incident1 = Incident.find(incident1.id)

test_label = '2_incidents_1_fr'
if incident0.state == 'incident_complete' && incident0.completion_status == 'normal'
  puts "Passed #{test_label}: #{incident0.id}"
else
  puts "Failed #{test_label}: #{incident0.id}"
  puts "\tstate:             #{incident0.state.to_i} - #{incident0.state}"
  puts "\tcompletion_status: #{incident0.completion_status.to_i} - #{incident0.completion_status}"
end

if incident1.state == 'incident_complete' && incident1.completion_status == 'no_frs'
  puts "Passed #{test_label}: #{incident1.id}"
else
  puts "Failed #{test_label}: #{incident1.id}"
  puts "\tstate:             #{incident1.state.to_i} - #{incident1.state}"
  puts "\tcompletion_status: #{incident1.completion_status.to_i} - #{incident1.completion_status}"
end
...
rp1.save
  ...
  puts "\tPart 9: incident0.id: #{incident0.id}"
  puts "\tPart 9: incident1.id: #{incident1.id}"
